...
TEST_CASE ("A fresh instance opens at 100% and its own design size", "[gui][scale]")
{
    ...
    CHECK_FALSE (processor.apvts.state.hasProperty (LancetAudioProcessorEditor::getScaleStatePropertyId()));
    CHECK (LancetAudioProcessorEditor::readPersistedScaleStepIndex (processor.apvts.state)
           == LancetAudioProcessorEditor::defaultScaleStepIndex);
...
TEST_CASE ("Applying a scale step resizes the window by exactly that factor", "[gui][scale]")
{
    ...
    for (size_t step = 0; step < LancetAudioProcessorEditor::scaleSteps.size(); ++step)
    {
        editor.applyScaleStep ((int) step);

        const auto scale = LancetAudioProcessorEditor::scaleSteps[step];
...
TEST_CASE ("Out-of-range step indices clamp instead of crashing", "[gui][scale]")
{
    ...
    CHECK (editor.getScaleStepIndex() == (int) LancetAudioProcessorEditor::scaleSteps.size() - 1);
}
...
TEST_CASE ("Continuous legacy scale values snap to the nearest step", "[gui][scale][state]")
{
    ...
    {
        processor.apvts.state.setProperty (LancetAudioProcessorEditor::getScaleStatePropertyId(),
...
TEST_CASE ("The chosen scale survives a full plugin-state round trip", "[gui][scale][state]")
{
    ...
    LancetAudioProcessor restored;
    restored.setStateInformation (savedState.getData(), (int) savedState.getSize());

    CHECK (LancetAudioProcessorEditor::readPersistedScaleStepIndex (restored.apvts.state) == 2);

    LancetAudioProcessorEditor editor (restored);
...
TEST_CASE ("Persisting the scale does not disturb parameter state", "[gui][scale][state]")
{
    ...
    auto* b1Gain = processor.apvts.getParameter ("b1_gain");
    REQUIRE (b1Gain != nullptr);

    b1Gain->setValueNotifyingHost (0.73f);

    LancetAudioProcessorEditor editor (processor);
    editor.applyScaleStep (3);

    CHECK (b1Gain->getValue() == Catch::Approx (0.73f).margin (1.0e-4));
}
