...
are research-derived starting points, not measured-hardware or
listening-comparison calibrated - see `docs/design-brief.md`'s and
`docs/voicing-notes.md`'s Honesty sections for exactly what that means.

| Preset | Category | Intent |
|---|---|---|
| **Default** | Init | The plugin's own out-of-the-box defaults (Band 3 on at a static, non-dynamic Bell; every other band off; every band's Q/Threshold/Attack/Release voiced per its role, see `docs/voicing-notes.md`), exposed as an explicit preset so there's always a one-click way back to it. Also this plugin's M2 default-resolution starting state. |
